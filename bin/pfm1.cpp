...
void list_pmu_events(pfm_pmu_t pmu) {
  ...
  for (i = pinfo.first_event; i != -1; i = pfm_get_event_next(i)) {
    ...
    for (int attr = 0; attr < info.nattrs; ++attr) {
      ...
      ret = pfm_get_event_attr_info(info.idx, attr, PFM_OS_PERF_EVENT,
                                    &eventAttr);
      if (ret != PFM_SUCCESS) {
        fprintf(stderr, "  ATTR: error: %s\n", pfm_strerror(ret));
      } else if (eventAttr.type == PFM_ATTR_UMASK) {
        // printf("umask: %s\n", eventAttr.name);
        names.push_back(std::string(info.name) + ":" + eventAttr.name);
      } else {
        printf("other attr: %s %s\n", eventAttr.name, eventAttr.desc);
      }
    }
    ...
    for (auto &name : names) {
      ...
      ret = pfm_get_os_event_encoding(name.c_str(), PFM_PLM0 | PFM_PLM3,
